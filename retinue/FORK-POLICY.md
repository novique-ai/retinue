...
| `tools/image_generation_tool.py` | `supports_upscale` / `upscale_note`: a plugin provider's `capabilities()` may declare that the backend honors the tool's `upscale` flag (its high-resolution pass is a different model), surfaced in the dynamic schema so agents learn to use it. Predates this table; registered retroactively (#144). Drift guards: `test_image_schema_surfaces_backend_declared_upscale` + `test_image_capabilities_pass_backend_upscale_through`. | [NousResearch/hermes-agent#90045](https://github.com/NousResearch/hermes-agent/issues/90045) |
...
| `cli.py`, `tui_gateway/slash_worker.py` | Slash-worker `/tools` joins in-flight MCP discovery (30s) so a slow stdio handshake is not listed as a missing server. The join is **gated to the slash worker** (`HERMES_SLASH_WORKER=1`, set in `_prepare_slash_worker_runtime`); an ungated join is an interactive regression — a human's `/tools` would block the full 30s on a hung MCP server, and a future sync that "simplifies" the gate away must be recognisable as wrong from this table. Slash workers have no TUI late-refresh, so without the join the first `/tools` answers with a catalog that's still missing the connecting server ([#193](https://github.com/novique-ai/retinue/issues/193)). Test-only companions (not product patches): `tests/tui_gateway/test_slash_worker_mcp_discovery.py` and `tests/cli/test_cli_tools_command.py`; inherited-lane flake tightening in `tests/tools/test_zombie_process_cleanup.py`, `tests/gateway/test_session_hygiene.py`, and `tests/run_agent/test_sequential_tool_timeout.py` rides along | [NousResearch/hermes-agent#92330](https://github.com/NousResearch/hermes-agent/issues/92330) |
| `tools/approval.py` | `approvals.room_mode` (default deny): room turns are gateway sessions whose humans never see approval UI, so the interactive gateway path submits a pending approval with no listener and blocks the turn for its full ~300s timeout ([#208](https://github.com/novique-ai/retinue/issues/208)). Mirrors the existing cron/single-query branches: `_is_room_workspace_approval_context()` (detects the rooms workspace overlay via fork-owned `tools/workspace_context.py`; cron keeps precedence) demotes the three approval flows to deterministic config resolution. Drift guards: `test_room_workspace_approval_context_detection`, `test_execute_code_denies_immediately_in_room_turns`, `test_dangerous_command_denies_fast_in_room_turns`, `test_cron_precedence_over_room_context` | [NousResearch/hermes-agent#92577](https://github.com/NousResearch/hermes-agent/issues/92577) |
| `tools/credential_files.py`, `tools/environments/docker.py` | `include_profile_skills` keyword on `get_skills_directory_mount` (default True) plus the docker call site that feeds it from `TERMINAL_DOCKER_SKIP_PROFILE_SKILLS_MOUNT` through `workspace_context`. Without the gate, a shared-room container always bind-mounts the creating profile's `~/.hermes/skills` at `/root/.hermes/skills`, so every member can read the creator's private skills ([#192](https://github.com/novique-ai/retinue/issues/192)). Non-room callers are unchanged | [NousResearch/hermes-agent#92329](https://github.com/NousResearch/hermes-agent/issues/92329) |
