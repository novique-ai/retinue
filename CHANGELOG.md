...
- Speak Replies always runs the semantic rewrite on a non-empty turn
  (no complexity skip). URLs are pointed at as “this link”, not read as
  domains or GitHub issue numbers. Code fences stay summarized. Unique
  retainer voices are unchanged. Closes #227.
...
- **Speech humanization for Speak Replies.** Rooms TTS no longer reads
  ...
  normalization, semantic rewrite via the auxiliary router, optional
  untrusted `spoken_summary`). Code fences are summarized, URLs are
  pointed at, env vars and paths are spoken as words, itinerary cards
  stay silent. Semantic rewrite failures fall back to the deterministic
  script; the humanizer cannot fail the TTS path. Docs: `retinue/VOICE.md`.
